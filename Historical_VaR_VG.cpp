...
#include <iostream>
#include <fstream>
double VG_Pricing(double alpha, double sigma, double v, double S, double K, double r, double T, string type) {
	...
	double call = S*exp(a*T / 2.0) * pow((1.0 - v*a / 2.0), T / v)*normcdf(d1)
...
void historical(vector<Future> futuresPortfolio, vector<vector<option>> optionsPortfolio, map<string, map<double, double> > impliedVol_grid) {
			...
			for (int i3 = 0; i3 < optionsPortfolio[i2].size(); ++i3) {
				for (int i4 = 0; i4 < historicalReturnData.nCol; ++i4) {
					if ((optionsPortfolio[i2][i3].underlying == historicalReturnData.title[i4]) && (optionsPortfolio[i2][i3].position != 0.0)) {
						...
						double imp_alpha, imp_sigma, imp_v;
						for (int k = 0; k < futuresPortfolio.size(); k++) {
							if (futuresPortfolio[k].title == optionsPortfolio[i2][i3].underlying) {
								imp_alpha = futuresPortfolio[k].implied_alpha;
								imp_sigma = futuresPortfolio[k].implied_sigma;
								imp_v = futuresPortfolio[k].implied_v;
							}
						}
						optionsPortfolio[i2][i3].priceEvaluatedByVG = VG_Pricing(imp_alpha, imp_sigma, imp_v, optionsPortfolio[i2][i3].MCunderlyingPrice, optionsPortfolio[i2][i3].maturity - (1.0 / 252.0), optionsPortfolio[i2][i3].rf,optionsPortfolio[i2][i3].maturity, optionsPortfolio[i2][i3].optionType);
						
						if (isnan(optionsPortfolio[i2][i3].priceEvaluatedByVG - optionsPortfolio[i2][i3].optionPrice)) {
							simulatedPortPriceDif[i1] += 0;
						}
						else {
							simulatedPortPriceDif[i1] += optionsPortfolio[i2][i3].position * (optionsPortfolio[i2][i3].priceEvaluatedByVG - optionsPortfolio[i2][i3].optionPrice);
						}
					}
				}
			}
	...
	*/
	...
	for (int i1 = 0; i1 < optionsPortfolio.size(); ++i1) {
		for (int i2 = 0; i2 < optionsPortfolio[i1].size(); ++i2) {
			output_infoOnOptions << to_string(optionsPortfolio[i1][i2].optionPrice) << ",";
		}
	}
	output_infoOnOptions << 0 << endl;
	...
	for (int i1 = 0; i1 < optionsPortfolio.size(); ++i1) {
		for (int i2 = 0; i2 < optionsPortfolio[i1].size(); ++i2) {
			portfolioPrice += optionsPortfolio[i1][i2].optionPrice * optionsPortfolio[i1][i2].position;
		}
	}
